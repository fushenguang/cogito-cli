...
- **Multi-level progression is data, since 0.9.0**: list as many entries as you like in `game-data.json`'s `levels` — reaching a level's goal advances to the next one (same Game scene, fresh geometry; `score` re-zeroes per level, `persistValues` carry across levels). The last level's goal ends the run (ending page, `cleared`). The machine-completion invariant still applies to `levels[0]` only, and for a multi-level project the selfcheck accepts "advanced to the next level" as its walk outcome — the ending page it pixel-asserts after that is reached via a harness jump, because a real player only sees it after beating every level, which is your acceptance territory, not the template's floor.
